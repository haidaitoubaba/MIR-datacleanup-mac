...
def analyse(source_path: Path, output: Path, properties: list[str] | str | None = None, bounds: dict | None = None,
    ...
    if prior:
        ...
        previous_payload = json.loads((prior / 'pca_data.json').read_text())
...
def apply_review(review_dir: Path, destination: Path, historical_replay: bool = False,
    ...
    payload = json.loads((review_dir / 'pca_baseline.json').read_text())
    for prop, data in payload.items():
...
def configure_modelling(cfg: dict) -> dict:
    ...
    if workbook:
        path = Path(workbook)
        provenance = json.loads(path.with_suffix('.cleanup.json').read_text())
        ...
        provenance = json.loads((Path(review_dir) / 'manifest.json').read_text())
...
def main() -> None:
    ...
    with threadpool_limits(limits=1):
        if args.command == 'analyse':
            analyse(args.source, args.output, 'all' if args.all_properties else args.properties, json.loads(args.bounds.read_text()) if args.bounds else None, args.prior)
        else:
