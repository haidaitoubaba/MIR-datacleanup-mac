...
_runtime_log = open(Path(tempfile.gettempdir(), 'mir-cleanup-runtime.log'), 'a', buffering=1)
sys.stdout = _runtime_log
sys.stderr = _runtime_log
faulthandler.enable(_runtime_log)

def startup_error(kind, value, tb):
    Path(tempfile.gettempdir(), 'mir-cleanup-startup-error.log').write_text(''.join(traceback.format_exception(kind,value,tb)))
