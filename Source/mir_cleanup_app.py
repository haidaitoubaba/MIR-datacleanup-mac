...
class ReadableComboBox(W.QComboBox):
    """Reserve room for text and the macOS arrow; size menus to their content."""
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setSizeAdjustPolicy(W.QComboBox.AdjustToContents)
        self.setMinimumContentsLength(10)
        self.setSizePolicy(W.QSizePolicy.MinimumExpanding, W.QSizePolicy.Fixed)
        self.setView(W.QListView())
    ...
    def sizeHint(self):
        size=super().sizeHint()
        width=max([self.fontMetrics().horizontalAdvance(self.itemText(i)) for i in range(self.count())]+[90])
        size.setWidth(max(size.width(),width+48))
        return size

    def showPopup(self):
        width=max([self.view().fontMetrics().horizontalAdvance(self.itemText(i)) for i in range(self.count())]+[90])
        self.view().setMinimumWidth(max(self.width(),width+48))
        super().showPopup()
...
class Setup(W.QDialog):
    ...
    def reject(self):
        if getattr(self,'reader',None):
            W.QMessageBox.information(self,'Reading workbook','Please finish the Mac file-access prompt, if shown, and wait for the workbook read to finish.');return
        super().reject()
    def accept(self):
        if getattr(self,'reader',None):
            return
...
class Window(W.QMainWindow):
    def __init__(self):
        ...
        self.prop=ReadableComboBox();self.prop.currentTextChanged.connect(self.refresh)
        self.stage=ReadableComboBox();self.stage.addItems(['PCA','A','B']);self.stage.currentTextChanged.connect(self.refresh)
        self.xpc=ReadableComboBox();self.ypc=ReadableComboBox();self.xpc.currentIndexChanged.connect(self.draw);self.ypc.currentIndexChanged.connect(self.draw)
        self.scale=ReadableComboBox();self.scale.addItems(['Modelled scale','Original units']);self.scale.currentIndexChanged.connect(self.draw)
        self.colour=ReadableComboBox();self.colour.addItems(['Status','Treatment','Reference value']);self.colour.currentIndexChanged.connect(self.draw)
        self.subset=ReadableComboBox();self.subset.addItems(['All in fitted model','Retained only','Excluded only']);self.subset.currentIndexChanged.connect(self.draw)
        for i,(label,widget) in enumerate([('Property',self.prop),('Stage',self.stage),('X PC',self.xpc),('Y PC',self.ypc),('Scale',self.scale),('Colour',self.colour),('Show',self.subset)]):
            row,col=divmod(i,4)
            controls.addWidget(W.QLabel(label),row,col*2);controls.addWidget(widget,row,col*2+1)
            controls.setColumnStretch(col*2+1,1)
        ...
        self.select_mode=ReadableComboBox();self.select_mode.addItems(['Click points','Rectangle selection','Lasso selection']);self.select_mode.currentIndexChanged.connect(self.selection_mode);self.toolbar.navigation_changed.connect(self.selection_mode)
        ...
        self.reason=ReadableComboBox();self.reason.addItems(['','redundancy','spectral_quality','range','concentration'])
        self.unit=ReadableComboBox();self.unit.addItems(['spectrum','sample'])
    ...
    if '--verify-session' in sys.argv:
        ...
        try:
            with tempfile.TemporaryDirectory(prefix='mir-bundle-check-') as temp:
                ...
                (output/'verification.json').write_text(json.dumps({'passed':True,'properties':list(session.state['properties'])}))
            window.close();return
        except Exception:
            (output/'verification-error.txt').write_text(traceback.format_exc());raise
    sys.exit(app.exec())
